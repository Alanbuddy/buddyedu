...
//        $left = $request->get('left', date('Y-m-d', strtotime('-1 week Monday')));
    ...
    public function statGroupByCourse(Request $request)
    {
        ...
        if ($isAdmin) {
            $query->addSelect(DB::raw('(select count(*) from schedules join schedule_user on schedules.id=schedule_user.schedule_id join courses on courses.id=schedules.course_id where schedule_user.type=\'student\' and schedules.end > date_format(now(),\'%Y-%m-%d %H:%i:%s\') and courses.id=cid ) as ongoingStudentCount'))
                ...
                ->addSelect(DB::raw('(select sum(round(amount/100,2)) from orders join schedules on schedules.id=product_id join courses on courses.id=schedules.course_id where courses.id=cid and orders.status=\'paid\') as income'))
                ->addSelect(DB::raw('(select sum(round(amount/100,2)) from orders join schedules on schedules.id=product_id join courses on courses.id=schedules.course_id where courses.id=cid and orders.status=\'paid\' and orders.created_at > \'' . $left . '\' and orders.created_at <\'' . $right . '\') as incomeOfSelectedRange'));
        } else {
    ...
    public function ongoingWithdrawApplicationsQuery(Merchant $merchant)
    {
        return $merchant->applications()->applying()
            ->withdrawType();
    }
